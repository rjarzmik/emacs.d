...
(defun dctrl-buffer-lookup (state)
  (let* ((bufs (delete-if-not (curry 'string-prefix-p dctrl-buf-prefix)
			      (buffer-list) :key 'buffer-name))
	 (stated (delete-if-not (lambda (x) (with-current-buffer x
					      (eq dctrl-state 'running))) bufs)))
    (cond ((= (length stated) 0) (error "No buffer found"))
	  ((= (length stated) 1) (car stated))
	  (get-buffer (ido-completing-read "Buffer: " (mapcar 'buffer-name stated))))))
  ...
  (mapcar (curry 'buffer-local-value 'dctrl-device-name)
	  (remove-if-not
	   (lambda (b)
	     (and
	      (or (not state)
		  (string= (buffer-local-value 'dctrl-state b) state))
	      (or (not backend-name)
		  (string= (dctrl-backend-name
			    (buffer-local-value 'dctrl-backend b))
			   backend-name))))
	   (dctrl-buffers))))
...
(defun dctrl-run-process (args)
  (lexical-let ((args args))
		(list (lambda ()
			...
			(set-process-sentinel (apply 'start-file-process "ctrl" (current-buffer) args)
...
(defun dctrl-run-local-process (args)
  (lexical-let ((args args))
		(list (lambda ()
			(dctrl-msg (format "local:%s" (mapconcat 'identity args " ")))
			(set-process-sentinel (apply 'start-process "ctrl"
						     (current-buffer) args)
					      'dctrl-process-sentinel)
			(set-process-filter (get-buffer-process (current-buffer))
					    'dctrl-process-filter)
